...
* secure - Булево - [НЕОБЯЗАТЕЛЬНЫЙ] использовать защищенное соединение по протоколу TLS 1.2. По-умолчанию Ложь. 
...
<b>GetPriority</b> - получает приоритет для последнего считанного сообщения. Имеет смыл вызывать внетри цикла чтения сообщений методом BasicConsumeMessage
...
<b>GetRoutingKey</b> - получает ключ маршрутизации для последнего считанного сообщения. Имеет смыл вызывать внетри цикла чтения сообщений методом BasicConsumeMessage
