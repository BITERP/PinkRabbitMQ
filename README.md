...
<b>Connect</b> - устанавливает соединение с сервером RabbitMQ. Если сервер недоступен или указаны  неверные логин, пароль или vhost, то будет возвращена ошибка, которая можеь быть получена через метод GetLastError;
