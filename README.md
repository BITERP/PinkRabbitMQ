⚡ **В БИТ:ERP открыты вакансии разработчиков 1С, [присоединяйся к нашей команде!](https://hh.ru/search/vacancy?area=1&search_field=description&text=%D0%91%D0%98%D0%A2%3AERP&fromSearchLine=false&st=searchVacancy)**
...
<b>BatchPublish</b> - отправить пакет сообщений в одну точку обмена

Параметры:
* exchange - Строка - имя точки обмена
* persist - Булево - сбрасывать сообщения на диск
* messages - Строка - JSON-массив сообщений. Пример:
...
[
  {
    "routingKey": "route1",
    "body": "Message 1",
    "properties": { "MessageId": "id-1", "AppId": "example" },
    "headers": { "x-custom": 42 }
  }
]
```

Поля `properties` и `headers` необязательны. В `properties` используются имена: CorrelationId, TypeName, MessageId, AppId, ContentEncoding, ContentType, UserId, ClusterId, Expiration, ReplyTo.
...
Параметры:
* messageTag - число - тег сообщения, который возвращается в параметре метода BasicConsumeMessage
* requeue - Булево - [НЕОБЯЗАТЕЛЬНЫЙ] вернуть сообщение в очередь. По умолчанию Ложь (сообщение удаляется из очереди).
...
<b>BasicCancel</b> - отменяет потребителя на сервере RabbitMQ и очищает локальный кеш сообщений

Параметры:
* channelId - Строка - имя потребителя, возвращённое методом BasicConsume. Пустая строка отменяет всех активных потребителей.
...
<b>GetQueueMessageCount</b> - возвращает количество сообщений в очереди (без их чтения)

Параметры:
* queue - Строка - имя очереди

Возвращаемое значение:
 - Число - количество сообщений в очереди на момент запроса
...
## Разворачивание окружения разработки на Windows (рекомендуется)

В репозитории уже есть скомпилированные зависимости в папке `lib/` для Win32 и x64.

1. Установить Visual Studio 2022 (Community) с компонентами «Разработка классических приложений на C++» и Windows SDK 10.
2. Клонировать репозиторий.
3. Открыть `windows/WindowsAddin.sln` или собрать из командной строки:

```powershell
cd windows
msbuild msbuild.xml -t:makeRelease -p:VERSION=2.2.0.local
```

Собранные DLL попадают в `package/` (`PinkRabbitMQ32.dll`, `PinkRabbitMQ64.dll`).

### Запуск тестов на Windows

Требуется запущенный RabbitMQ на `127.0.0.1:5672` (логин/пароль `guest`/`guest`).

```powershell
cd windows
msbuild msbuild.xml -t:runTests -p:VERSION=ci
```
...
cmake -DCMAKE_BUILD_TYPE=Release -DVERSION=ci -DNAME_POSTFIX= ..
cmake --build .
pip install pytest
ctest . -V
```
