...
## API
Компонента имеет ряд метод для работы из встроенного языка 1С. Параметры с тегом [НЕ РЕАЛИЗОВАНО] имеют пустую реализацию, но передавать их все равно требуется. Параметры с тегом [Необязательный] можно не передавать. Список методов:
